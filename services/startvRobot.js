...
class ReceitanetRobotService {
    ...
    async cadastrarEAtivarTV(cliente, loginTvInput, senhaTvInput) {
        ...
        console.log(`[STARTV-ROBOT] 🚀 Criando usuário '${loginTv}' | Nome: '${nomeCliente}' | CPF/Senha: '${cpf}'...`);

        try {
            // ============================================================
            // ETAPA 1: CADASTRO DO CLIENTE EM clientes_cadastro.php
            // ============================================================
            console.log(`[STARTV-ROBOT] ETAPA 1: Acessando a tela de cadastro...`);
            ...
            console.log(`[STARTV-ROBOT] ETAPA 1 ✅ Concluída. URL pós-criação: ${urlPosCriacao}`);
            ...
            // ============================================================
            console.log(`[STARTV-ROBOT] ETAPA 2: Descobrindo ID numérico do cliente recém-criado...`);
            ...
            if (!clienteId) {
                ...
                console.log(`[STARTV-ROBOT] Abrindo ficha via login para extrair ID: ${fichaUrl}`);
                ...
                if (extractResult) {
                    ...
                    console.log(`[STARTV-ROBOT] ID extraído da ficha do cliente: ${clienteId}`);
                }
            }

            // Método C: Busca no Novo ERP pela lista de clientes
            if (!clienteId) {
                console.log(`[STARTV-ROBOT] Buscando cliente '${loginClean}' no Novo ERP para obter ID...`);
            ...
            if (clienteId) {
                ...
                console.log(`[STARTV-ROBOT] ⚠️ ID não encontrado. Usando fallback legacy: ${planosPageUrl}`);
            }
            ...
            if (!selectApareceu) {
                console.log(`[STARTV-ROBOT] ⚠️ Select não apareceu. Aguardando 3s adicionais e tentando novamente...`);
            ...
            if (!cdntvInfo) {
                ...
                throw new Error(`[STARTV-ROBOT] Opção CDNTV não encontrada em nenhum select. URL: ${page.url()}. Selects: ${JSON.stringify(selectDiag)}`);
            }

            console.log(`[STARTV-ROBOT] Opção CDNTV encontrada → Select: '${cdntvInfo.selectName || cdntvInfo.selectId}', Valor: '${cdntvInfo.optionValue}', Texto: '${cdntvInfo.optionText}'`);
            ...
            console.log(`[STARTV-ROBOT] CDNTV selecionado. Clicando no botão INCLUIR...`);
            ...
            if (!clicouIncluir) {
                throw new Error(`[STARTV-ROBOT] Nenhum botão INCLUIR encontrado na página de planos. URL: ${page.url()}`);
            }
            ...
            console.log(`[STARTV-ROBOT SUCCESS] 🎉 CLIENTE '${loginTv}' CRIADO E PLANO CDNTV INCLUÍDO COM SUCESSO! URL final: ${page.url()}`);
            return true;

        } catch (error) {
            console.error(`[STARTV-ROBOT ERROR] Falha no cadastro + inclusão CDNTV:`, error.message);
