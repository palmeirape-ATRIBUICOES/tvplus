...
class ReceitanetRobotService {
    ...
    async excluirCliente(login, cpf, nome) {
        ...
        console.log('[RECEITANET-ROBOT] 🗑️ REINICIANDO LÓGICA DE EXCLUSÃO para cliente: ' + rawLogin + '...');
        ...
        const setupDialogHandler = (targetPage) => {
            ...
            targetPage.on('dialog', async (dialog) => {
                console.log('[RECEITANET-ROBOT] 💬 Confirmando diálogo popup: "' + dialog.message() + '"');
        ...
        try {
            ...
            console.log('[RECEITANET-ROBOT] PASSO 1: Acessando ' + cadastroUrl);
            ...
            console.log('[RECEITANET-ROBOT] Clicando na aba/opção Rescisão...');
            ...
            if (clicouRescisao) {
                ...
                console.log('[RECEITANET-ROBOT] Navegando diretamente para a URL de rescisão: ' + rescisaoUrl);
            ...
            // ============================================================
            console.log('[RECEITANET-ROBOT] PASSO 2: Selecionando "cancelado chip" e escrevendo "ok" em detalhe...');
            await page.waitForSelector('select', { timeout: 10000 });
            ...
            // ============================================================
            console.log('[RECEITANET-ROBOT] PASSO 3: Clicando no botão Calcular via XPath /html/body/div/div/section[2]/form/div[3]/button[1]...');
            ...
            // ============================================================
            console.log('[RECEITANET-ROBOT] PASSO 4: Fechando a nova guia aberta...');
            const targetGuia = await novaGuiaPromise;
            if (targetGuia) {
                try {
                    const pageGuia = await targetGuia.page();
                    if (pageGuia && !pageGuia.isClosed()) {
                        console.log('[RECEITANET-ROBOT] Fechando guia secundária (' + pageGuia.url() + ')...');
            ...
            // ============================================================
            console.log('[RECEITANET-ROBOT] PASSO 5: Voltando para ' + cadastroUrl);
            await page.goto(cadastroUrl, { waitUntil: 'domcontentloaded' });

            // ============================================================
            // PASSO 6: Ao carregar a página aperta F5 reload para aparecer o botão Excluir
            // ============================================================
            console.log('[RECEITANET-ROBOT] PASSO 6: Apertando F5 (reload) para liberar o botão Excluir...');
            ...
            if (clicouExcluir) {
                console.log('[RECEITANET-ROBOT] Botão Excluir acionado (' + clicouExcluir + '). Aguardando navegação...');
                await page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
                console.log('[RECEITANET-ROBOT SUCCESS] 🎉 CLIENTE ' + rawLogin + ' EXCLUÍDO COM SUCESSO!');
            } else {
                console.log('[RECEITANET-ROBOT] Botão Excluir não encontrado na página.');
            }
