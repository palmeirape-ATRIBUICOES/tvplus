...
class ReceitanetRobotService {
    ...
    async cadastrarEAtivarTV(cliente, loginTvInput, senhaTvInput) {
        ...
        console.log('[RECEITANET-ROBOT] 🚀 [VELOCIDADE ULTRA] Criando usuario ' + loginTv + ' | CPF/Senha: ' + cpf + '...');
        ...
        try {
            ...
            console.log('[RECEITANET-ROBOT] Campos preenchidos instantaneamente. Clicando em INCLUIR...');
            ...
            console.log('[RECEITANET-ROBOT] ETAPA 1 OK (' + ((Date.now() - startTime)/1000).toFixed(2) + 's). URL pós-criação: ' + urlPosCriacao);
            this.tirarScreenshot(page, '02_cliente_criado').catch(() => {});

            // ETAPA 2: EXTRAÇÃO INSTANTÂNEA DO ID NUMÉRICO DO CLIENTE
            console.log('[RECEITANET-ROBOT] ETAPA 2: Extraindo ID numérico...');
            ...
            if (!clienteId) {
                ...
                if (clienteId) {
                    console.log('[RECEITANET-ROBOT] ID extraído do DOM pós-criação sem recarregar: ' + clienteId);
                }
            }
            ...
            if (!clienteId) {
                for (const tentativaLogin of [loginTv, loginClean]) {
                    ...
                    console.log('[RECEITANET-ROBOT] Método B consulta rápida: ' + tentativaLogin);
                    ...
                    if (extractResult) {
                        ...
                        console.log('[RECEITANET-ROBOT] ID extraído (login=' + tentativaLogin + '): ' + clienteId);
                        break;
                    }
                }
            }
            ...
            if (!clienteId) {
                throw new Error('[RECEITANET-ROBOT] Não foi possível localizar o ID numérico do cliente ' + loginTv);
            }
            ...
            if (!cdntvInfo) {
                ...
                throw new Error('[RECEITANET-ROBOT] Opção CDNTV não encontrada nos selects da página de planos.');
            }
            ...
            console.log('[RECEITANET-ROBOT] Clicando no botão INCLUIR via XPath exato /html/body/div/div[1]/section[2]/div/div[2]/form/div[3]/button...');
            ...
            console.log('[RECEITANET-ROBOT SUCCESS] ⚡⚡ CONCLUÍDO EM APENAS ' + totalTimeSec + 's! Cliente ' + loginTv + ' criado e CDNTV ativado!');
            return true;

        } catch (error) {
            console.error('[RECEITANET-ROBOT ERROR] Falha na automação ultrarrápida:', error.message);
    ...
    async excluirCliente(login, cpf, nome) {
        ...
        console.log('[RECEITANET-ROBOT] 🗑️ Iniciando exclusão estrita do cliente: ' + rawLogin + ' (loginSemDominio: ' + loginSemDominio + ')...');
        ...
        const setupDialogHandler = (targetPage) => {
            ...
            targetPage.on('dialog', async (dialog) => {
                console.log('[RECEITANET-ROBOT] 💬 Diálogo popup detectado ("' + dialog.message() + '"). Confirmando (accept)...');
        ...
        try {
            ...
            console.log('[RECEITANET-ROBOT] PASSO 0: Selecionando o cliente no ERP (' + rawLogin + ')...');
            ...
            console.log('[RECEITANET-ROBOT] PASSO 1: Acessando caminho da rescisão: ' + rescisaoUrl);
            await page.goto(rescisaoUrl, { waitUntil: 'domcontentloaded' });
            await page.waitForSelector('select', { timeout: 10000 });

            // PASSO 2: Motivo do cancelamento -> "cancelado chip" | Detalhe -> "ok"
            console.log('[RECEITANET-ROBOT] PASSO 2: Selecionando motivo "cancelado chip" no dropdown e escrevendo "ok" em detalhe...');
            await page.evaluate(() => {
            ...
            console.log('[RECEITANET-ROBOT] PASSO 3: Clicando no botão /html/body/div/div/section[2]/form/div[3]/button[1]...');
            ...
            console.log('[RECEITANET-ROBOT] PASSO 4: Aguardando nova guia abrir para fechá-la...');
            const newTarget = await targetCreatedPromise;
            if (newTarget) {
                try {
                    const newPage = await newTarget.page();
                    if (newPage && !newPage.isClosed()) {
                        console.log('[RECEITANET-ROBOT] Nova guia detectada (' + newPage.url() + '). Fechando guia...');
            ...
            console.log('[RECEITANET-ROBOT] PASSO 5: Voltando ao cadastro do cliente: ' + cadastroUrl);
            ...
            console.log('[RECEITANET-ROBOT] PASSO 6: Utilizando F5 (reload) para atualizar a página e liberar o botão Excluir...');
            ...
            console.log('[RECEITANET-ROBOT] PASSO 7: Clicando no botão Excluir via XPath /html/body/div/div[1]/section[2]/div[2]/div[1]/form/div[1]/div[2]/button[3]...');
            ...
            if (clicouExcluir) {
                console.log('[RECEITANET-ROBOT] Botão Excluir acionado (' + clicouExcluir + '). Aguardando confirmação final...');
                await page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
                console.log('[RECEITANET-ROBOT SUCCESS] 🎉 CLIENTE ' + rawLogin + ' EXCLUÍDO COM SUCESSO!');
            } else {
                console.log('[RECEITANET-ROBOT] Aviso: Botão Excluir não localizado. Verificando se já foi removido.');
            }
            ...
            console.error('[RECEITANET-ROBOT ERROR] Falha na exclusão do cliente ' + rawLogin + ':', error.message);
