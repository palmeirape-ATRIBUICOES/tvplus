...
class ReceitanetRobotService {
    ...
    async excluirCliente(login, cpf, nome) {
        ...
        try {
            ...
            console.log('[RECEITANET-ROBOT] PASSO 2: Selecionando motivo cancelado chip e detalhe ok...');
            await page.waitForSelector('select[name="cancelamento_motivo"], select', { timeout: 10000 });
            ...
            try {
                ...
                if (pages && pages.length > 1) {
                    console.log('[RECEITANET-ROBOT] PASSO 4: Fechando ' + (pages.length - 1) + ' guia(s) popup secundaria(s)...');
            ...
            if (!clicouExcluir) {
                console.log('[RECEITANET-ROBOT] Aviso: Botao Excluir nao encontrado na ficha. Verificando se ja foi excluido.');
            } else {
                await page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
                console.log('[RECEITANET-ROBOT SUCCESS] 🎉 Cliente ' + rawLogin + ' excluido com sucesso via ' + clicouExcluir + '!');
            }
            ...
            console.error('[RECEITANET-ROBOT ERROR] Falha na exclusao do cliente ' + rawLogin + ':', error.message);
